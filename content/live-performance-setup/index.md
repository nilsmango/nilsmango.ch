...
For me, performing live is all about being able to immediately realize any musical idea. That starts long before choosing the right [controller](#the-ableton-live-controller). It starts with getting the basics right, building a solid live set, collecting great presets, and then practicing until everything becomes second nature.
...
I use external cameras if available (if I am on the road, I simply use the webcam in my MacBook). Usually I have one top view of my controller and one from a bit farther away with myself in the frame to keep things visually interesting. To make it even more interesting, I use an [automatic scene switching script](https://gist.github.com/nilsmango/58857855454801f4bc642f74dfffdaef) inside OBS. Add some good lighting if possible. It makes a huge difference to video quality.
...
I have a few master effects on the master channel (for drops and global effects). 
Track one is my drum track. That is often followed by another percussion or vocal track (both typically drum racks), then bass, lead, pad, and so on. Each track but the drum track has a compressor with sidechain tuned to the kick frequency of my drum track.
...
### Drum Tips
Do not make the individual drum pads too loud. You can always push them up later. I recommend a grouped instrument with volume control on each pad. I also like adding global effects to individual drum pads, for example delay and reverb sends. To get send controls inside a drum pad, you need a small workaround:

1. Add the modulator `Shaper` into the drum pad group.
2. Switch the Shaper to `Manual` mode.
3. Map the Shaper to the send of that pad for the reverb or delay (I use Remote, not Modulate).
4. Make the Shaper use a linear shape (or whatever you prefer).
...
>  
> If you are not convinced, check this [comparison page for the best Ableton Live controllers in 2026](https://project7iii.com/tap/best-ableton-live-controller/) (also written by yours truly) to see where Tap shines compared to Push and other controller apps.
